...
stat([X|L], Reste, Sugg) :- X="if", paren_expr(L, R1, Sugg1), paren_expr(R1, R2, Sugg2), Reste=R2, append_all([Sugg1, Sugg2], Sugg).
stat([X|L], Reste, Sugg) :- X="if", paren_expr(L, R1, Sugg1), paren_expr(R1, R2, Sugg2), else(R2, R3, Sugg3), Reste=R3, append_all([Sugg1, Sugg2, Sugg3], Sugg).
else([], Reste, Sugg) :- else_words(W), else([W], Reste, Sugg1), append([W], Sugg1, Sugg).
else([X|L], Reste, Sugg) :- X="else", paren_expr(L, Reste, Sugg).
